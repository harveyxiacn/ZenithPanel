export default {
  // Common
  common: {
    loading: 'Loading...',
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    add: 'Add',
    close: 'Close',
    confirm: 'Confirm',
    back: 'Back',
    done: 'Done',
    refresh: 'Refresh',
    actions: 'Actions',
    enabled: 'Enabled',
    disabled: 'Disabled',
    active: 'Active',
    configure: 'Configure',
    change: 'Change',
    errorOccurred: 'An error occurred',
    saved: 'Saved successfully',
    deleted: 'Deleted successfully',
    created: 'Created successfully',
    applied: 'Configuration applied',
    areYouSure: 'Are you sure?',
    all: 'All',
    exportCsv: 'Export CSV',
    exported: 'Exported',
    exportFailed: 'Export failed',
    lightMode: 'Light Mode',
    darkMode: 'Dark Mode',
  },

  // Navigation / Layout
  nav: {
    dashboard: 'Dashboard',
    servers: 'Servers & Files',
    proxyNodes: 'Proxy Nodes',
    usersSubs: 'Users & Subs',
    security: 'Security',
    logout: 'Logout',
    sites: 'Sites',
    traffic: 'Traffic',
    egress: 'Egress',
    smartDeploy: 'Smart Deploy',
  },

  usageProfile: {
    personalProxy: {
      label: 'Personal Proxy',
      description: 'Lead with nodes, clients, subscriptions, and endpoint health.',
      emphasis: 'Best for personal devices, family, or a small private group.',
    },
    vpsOps: {
      label: 'VPS Ops',
      description: 'Lead with monitoring, terminal, files, Docker, and firewall.',
      emphasis: 'Best when ZenithPanel is primarily your server operations surface.',
    },
    mixed: {
      label: 'Mixed',
      description: 'Balance proxy operations and VPS management in one workspace.',
      emphasis: 'Best when you actively use both sides of the panel.',
    },
  },

  // Login
  login: {
    title: 'Login to Zenith',
    username: 'Username',
    password: 'Password',
    submit: 'Login',
    errorEmpty: 'Please enter both username and password',
    errorRateLimit: 'Too many login attempts. Please try again later.',
    errorLocked: 'Too many failed attempts. Try again in {n} minutes.',
    totpCode: '2FA Verification Code',
    totpPlaceholder: '000000',
    totpHint: 'Enter the 6-digit code from your authenticator app, or a recovery code.',
    enterTOTP: 'Please enter your 2FA code',
  },

  // Setup Wizard
  setup: {
    title: 'ZenithPanel',
    subtitle: 'Secure Initialization Wizard',
    welcomeMsg: 'Welcome to your new instance! To protect against unauthorized scanning, please enter the temporary password generated in your server console.',
    initialPassword: 'Initial One-Time Password',
    verifyIdentity: 'Verify Identity',
    identityVerified: 'Identity Verified. Configure Panel Security.',
    adminUsername: 'Admin Username',
    newAdminPassword: 'New Admin Password',
    confirmPassword: 'Confirm Password',
    minChars: 'Minimum 8 characters',
    retypePassword: 'Re-type new password',
    customPanelPath: 'Custom Panel URL Path',
    usageProfile: 'Usage Profile',
    usageProfileHint: 'This changes which workflows ZenithPanel emphasizes first. You can change it later in Settings.',
    customPanelPathHint: 'Change to prevent automated scanning.',
    applyInit: 'Apply & Initialize Zenith',
    completeTitle: 'Initialization Complete!',
    completeMsg: 'The temporary setup token has been destroyed. Your panel is now securely locked.',
    bookmarkMsg: 'Please bookmark your new panel URL:',
    goToLogin: 'Go to Login',
    errorPasswordMismatch: 'Passwords do not match',
    errorPasswordShort: 'New password must be at least 8 characters',
    errorUsernameLength: 'Username must be between 3 and 32 characters',
  },

  // Dashboard
  dashboard: {
    title: 'System Overview',
    subtitle: 'Real-time performance metrics and proxy status.',
    profileOverview: 'Profile-aware workspace',
    profileHint: 'ZenithPanel keeps one shared feature set, but changes the order, emphasis, and recommended actions to fit your selected usage profile.',
    quickActions: 'Quick Actions',
    cpuUsage: 'CPU Usage',
    memory: 'Memory',
    diskUsage: 'Disk Usage',
    network: 'Network',
    up: 'Up',
    systemInfo: 'System Info',
    quickStats: 'Quick Stats',
    uptime: 'Uptime',
    loadAverage: 'Load Average',
    hostname: 'Hostname',
    cpu: 'CPU',
    disk: 'Disk',
    customizeCards: 'Customize Cards',
    profile: {
      primaryActionBadge: 'Recommended',
      personalProxy: {
        title: 'Proxy-first overview',
        description: 'Jump back into nodes, subscriptions, and diagnostics without digging through server tooling first.',
        primaryAction: 'Open Proxy Nodes',
        primaryActionHint: 'Manage inbounds, routing, apply, and diagnostics.',
        secondaryActionClients: 'Manage Clients & Subs',
        secondaryActionClientsHint: 'Add users, copy links, and rotate subscriptions.',
        secondaryActionSecurity: 'Review Security Settings',
        secondaryActionSecurityHint: 'Update panel access, TLS, and profile preferences.',
      },
      vpsOps: {
        title: 'Server-first overview',
        description: 'Keep core VPS operations in front while still retaining proxy tools when you need them.',
        primaryAction: 'Open Servers & Files',
        primaryActionHint: 'Go to terminal, files, Docker, and firewall workflows.',
        secondaryActionSecurity: 'Review Security Settings',
        secondaryActionSecurityHint: 'Manage access, updates, TLS, and optimization.',
        secondaryActionProxy: 'Open Proxy Nodes',
        secondaryActionProxyHint: 'Jump into proxy operations when server work is done.',
      },
      mixed: {
        title: 'Balanced overview',
        description: 'Keep proxy and VPS operations side by side without overcommitting the interface to either side.',
        primaryAction: 'Open Dashboard',
        primaryActionHint: 'Stay on the combined overview with balanced system context.',
        secondaryActionServers: 'Open Servers & Files',
        secondaryActionServersHint: 'Go straight to operations, Docker, and filesystem tools.',
        secondaryActionNodes: 'Open Proxy Nodes',
        secondaryActionNodesHint: 'Go straight to inbounds, clients, and subscriptions.',
      },
    },
  },

  // Servers
  servers: {
    title: 'Servers & Files',
    subtitle: 'Manage underlying VPS, file system, and Docker engine.',
    tabs: {
      terminal: 'Web Terminal',
      files: 'File Explorer',
      docker: 'Containers',
      firewall: 'Firewall',
    },
    terminal: {
      connecting: 'Connecting...',
      connectingToShell: 'Connecting to shell...',
    },
    files: {
      loadingFiles: 'Loading files...',
      emptyDir: 'Empty directory',
      saving: 'Saving...',
    },
    docker: {
      title: 'Docker Engine',
      loadingContainers: 'Loading containers...',
      container: 'Container',
      image: 'Image',
      status: 'Status',
      running: 'Running',
      exited: 'Exited',
      start: 'Start',
      stop: 'Stop',
      restart: 'Restart',
      remove: 'Remove',
      noContainers: 'No containers found',
      confirmRemove: 'Remove this container?',
    },
    firewall: {
      title: 'Firewall Rules',
      subtitle: 'Manage iptables INPUT chain rules.',
      addRule: 'Add Rule',
      port: 'Port',
      sourceIp: 'Source IP (opt)',
      target: 'Target',
      protocol: 'Protocol',
      source: 'Source',
      loadingRules: 'Loading rules...',
      noRules: 'No rules found (iptables may not be available)',
      confirmDelete: 'Delete this rule?',
    },
  },

  // Proxy
  proxy: {
    confirmDeleteInbound: 'This inbound and all its settings will be permanently deleted.',
    confirmDeleteClient: 'This client will lose access to the proxy.',
    confirmDeleteRule: 'This routing rule will be removed.',
    title: 'Proxy Services',
    subtitle: 'Configure Xray/Sing-box engine, routing rules, and user subscriptions.',
    applyConfig: 'Apply Configuration',
    testConnection: 'Check Server Network',
    status: {
      xrayRunning: 'Xray Running',
      xrayStopped: 'Xray Stopped',
      nodes: 'Nodes: {n}',
      users: 'Users: {n}',
      rules: 'Rules: {n}',
      dualMode: 'Dual engines',
      dualModeHint: 'Xray and Sing-box run side-by-side; each engine owns a disjoint set of inbounds.',
      skipped: '{n} protocol(s) skipped',
      skippedHint: 'Xray skipped these protocols (switch to Sing-box engine to serve them): {list}',
      handedOff: '{n} served by Sing-box',
      handedOffHint: 'These protocols are served by Sing-box in dual-engine mode: {list}',
    },
    tabs: {
      inbounds: 'Inbound Nodes',
      routing: 'Routing Rules',
      users: 'User & Subs',
    },
    inbounds: {
      title: 'Inbound Listeners',
      quickSetup: 'Quick Setup',
      addNode: 'Add Node',
      editInbound: 'Edit Inbound',
      addInbound: 'Add Inbound',
      tag: 'Tag',
      protocol: 'Protocol',
      port: 'Port',
      transport: 'Transport',
      listen: 'Listen',
      serverAddress: 'Public Host / IP',
      settingsJson: 'Settings (JSON) - Protocol config',
      streamJson: 'Stream (JSON) - Transport & TLS',
      noInbounds: 'No inbounds configured yet',
      oneClickConfig: 'One-click recommended configuration',
      confirmDelete: 'Delete this inbound?',
      visual: 'Visual',
      network: 'Network',
      security: 'Security',
      fingerprint: 'Fingerprint',
      certFile: 'Certificate File',
      keyFile: 'Key File',
      destSni: 'Dest (SNI target)',
      serverNames: 'Server Names',
      privateKey: 'Private Key',
      publicKey: 'Public Key',
      regenKeys: 'Regenerate Keys',
      wsPath: 'WS Path',
      wsHost: 'WS Host',
      grpcService: 'gRPC Service Name',
      ssMethod: 'Encryption Method',
      ssPassword: 'Password',
      addUser: 'Add User',
      probe: 'Probe',
      probeTooltip: 'Run a server-side connectivity check on this inbound',
      probeRecheck: 'click to re-check',
      probeAll: 'Probe all',
      probingAll: 'Probing…',
      probeAllTooltip: 'Probe every enabled inbound; results appear inline per row',
      autoProbeOff: 'Auto-probe off',
      autoProbeOn: 'Auto-probing 30s',
      autoProbeTooltip: 'Re-probe every 30 seconds while the inbound tab is open',
      tlsRequiredHint: 'Hysteria2 and TUIC always require TLS — the SNI must match the certificate. No real domain? Use <your-IP-with-dashes>.nip.io.',
      singboxOnly: 'This protocol is supported only by Sing-box and runs on the Sing-box engine.',
    },
    routing: {
      title: 'Routing Rules',
      subtitle: 'Direct traffic to dedicated outbound nodes.',
      addRule: 'Add Rule',
      errors: {
        outboundRequired: 'Outbound tag is required',
        filterRequired: 'Fill at least one of Domain, IP, or Port — rules need something to match on.',
      },
      ruleName: 'Rule Name',
      outboundTag: 'Outbound Tag (direct/block)',
      name: 'Name',
      domain: 'Domain',
      domainPlaceholder: 'Domain (geosite:cn)',
      ipPlaceholder: 'IP (geoip:private)',
      outbound: 'Outbound',
      noRules: 'No routing rules configured',
      confirmDelete: 'Delete this routing rule?',
      addRecommended: 'Add Recommended',
      presets: 'Quick Add Presets:',
    },
    clients: {
      title: 'Client Management',
      addClient: 'Add Client',
      email: 'Email',
      uuid: 'UUID',
      traffic: 'Traffic',
      status: 'Status',
      trafficLimit: 'Traffic Limit (bytes)',
      selectInbound: 'Select Inbound',
      subLink: 'Sub Link',
      copied: 'Copied!',
      qrCode: 'QR Code',
      noClients: 'No clients configured',
      confirmDelete: 'Delete this client?',
      unlimited: 'Unlimited',
    },
    subscription: {
      title: 'Subscription Link',
      urlLabel: 'Subscription URL',
      v2rayHint: 'Use the explicit Base64 subscription URL for V2RayN, v2rayNG, Nekoray, and similar V2Ray clients.',
      clashHint: 'Use the explicit Clash/YAML subscription URL for Clash, Mihomo, Stash, and similar clients.',
    },
    quickSetup: {
      title: 'Quick Setup',
      steps: { select: 'Select', review: 'Review', done: 'Done' },
      selectDesc: 'Choose one or more configurations. We recommend {recommended} for the best security and performance.',
      recommended: 'VLESS + Reality',
      useRecommended: 'Use Recommended',
      continueN: 'Continue ({n} selected)',
      reviewDesc: 'Everything is pre-configured with recommended values. Expand each node to customize settings if needed.',
      creating: 'Creating...',
      createAll: 'Create All',
      completeTitle: 'Setup Complete!',
      completeDesc: 'Your proxy nodes are configured and ready to use.',
      created: 'Created',
      failed: 'Failed',
      applyHint: 'Add clients, adjust settings, or click {apply} to activate.',
      additionalSetup: 'Additional Setup',
      addRoutingRules: 'Add recommended routing rules (block ads & private IPs)',
      createFirstClient: 'Create first client',
      // Preset names
      presets: {
        'vless-reality': { name: 'VLESS + Reality', desc: 'Most censorship-resistant. Disguises traffic using Reality fingerprint.' },
        'vless-ws-tls': { name: 'VLESS + WS + TLS', desc: 'Works behind CDN like Cloudflare. Great for restricted networks.' },
        'vmess-ws-tls': { name: 'VMess + WS + TLS', desc: 'Classic V2Ray protocol. Supported by virtually all clients.' },
        'trojan-tls': { name: 'Trojan + TLS', desc: 'Simple and fast. Mimics regular HTTPS traffic.' },
        'hysteria2': { name: 'Hysteria2', desc: 'UDP-based QUIC protocol. Best for high-speed, high-latency networks.' },
        'tuic': { name: 'TUIC v5', desc: 'Low-latency QUIC protocol with native UDP relay. Sing-box engine only.' },
        'shadowsocks': { name: 'Shadowsocks', desc: 'Simple encryption proxy. Wide client support, easy to deploy.' },
      },
      badges: {
        recommended: 'Recommended',
        cdnFriendly: 'CDN Friendly',
        wideSupport: 'Wide Support',
        simpleFast: 'Simple & Fast',
        ultraFast: 'Ultra Fast',
        lightweight: 'Lightweight',
      },
      errors: {
        publicHostRequired: '{preset}: please fill either Public Host / IP or Domain before applying — clients need it to reach the proxy.',
      },
      fields: {
        defaultPort: 'Default port',
        serverAddress: 'Public Host / IP',
        destSni: 'Destination (SNI Target)',
        serverNames: 'Server Names',
        privateKey: 'Private Key (auto-generated)',
        publicKey: 'Public Key (auto-generated)',
        shortId: 'Short ID',
        fingerprint: 'Fingerprint',
        domain: 'Domain (for TLS certificate)',
        domainRequired: 'Required for TLS. Use your domain pointed to this server.',
        publicHostHint: 'Fill either Public Host/IP or Domain below — clients use this value to reach the proxy.',
        certFile: 'Certificate File',
        keyFile: 'Key File',
        wsPath: 'WebSocket Path',
        encryptionMethod: 'Encryption Method',
        password: 'Password (auto-generated)',
      },
    },
    qr: {
      title: 'Subscription QR Code',
      v2ray: 'V2Ray / V2RayN',
      clash: 'Clash / Mihomo',
      generating: 'Generating...',
      scanWith: 'Scan with {clients}',
      v2rayClients: 'V2RayN, V2RayNG, or Shadowrocket',
      clashClients: 'Clash, Mihomo, or Stash',
      downloadPng: 'Download PNG',
      copyLink: 'Copy Link',
      v2rayHint: 'v2rayNG (Android) / Shadowrocket (iOS) / V2RayN (Windows) / Nekoray (Linux)',
      clashHint: 'Clash Mi / Stash (iOS) / Clash Verge / Mihomo Party (Desktop) / ClashMeta (Android)',
    },
  },

  // Security
  security: {
    confirmDisable2FA: 'Two-factor authentication will be disabled.',
    confirmRemoveTLS: 'HTTPS will be disabled and certificates removed.',
    confirmRemoveSwap: 'Swap memory will be removed.',
    title: 'Security & Settings',
    subtitle: 'Configure panel security, manage updates, and customize settings.',
    update: {
      title: 'Panel Update',
      subtitle: 'Check for and install updates automatically',
      currentVersion: 'Current Version',
      currentImage: 'Current Image',
      latestImage: 'Latest Image',
      available: 'A new version is available. Click "Update Now" to apply.',
      upToDate: 'You are running the latest version.',
      checking: 'Checking...',
      checkForUpdates: 'Check for Updates',
      updating: 'Updating...',
      updateNow: 'Update Now',
      confirmRestart: 'This will restart the panel. Continue?',
      restarting: 'Panel restarting... reloading in {n}s',
    },
    access: {
      title: 'Access Configuration',
      subtitle: 'Customize how you connect to ZenithPanel',
      usageProfile: 'Usage Profile',
      usageProfileHint: 'Reorders navigation, dashboard emphasis, and recommendations without disabling any features.',
      securityPath: 'Security Path Suffix',
      panelPort: 'Panel Port',
      portHint: 'Use a Cloudflare-compatible HTTPS port: 443, 2053, 2083, 2087, 2096, 8443',
      pathHint: 'Custom URL path to prevent automated scanning. Leave empty to disable.',
      applyRestart: 'Save & Restart',
      confirmRestart: 'This will restart the panel with the new port. You will be redirected automatically. Continue?',
      apiWhitelist: 'API White-list',
      apiWhitelistDesc: 'Restrict backend API access to specific IPs.',
    },
    tls: {
      title: 'HTTPS / TLS',
      subtitle: 'Secure panel connections with SSL certificates',
      statusEnabled: 'HTTPS Enabled',
      statusDisabled: 'HTTP Only',
      uploadCert: 'Certificate File (.pem / .crt)',
      uploadKey: 'Private Key File (.pem / .key)',
      uploadBtn: 'Upload & Enable',
      removeBtn: 'Remove TLS',
      restartNote: 'Panel restart required after uploading certificates.',
      confirmRemove: 'Remove TLS certificates? Panel will revert to HTTP after restart.',
      selectFiles: 'Please select both certificate and key files.',
      cfGuideTitle: 'Cloudflare Origin Certificate Guide',
      cfGuideIntro: 'Free HTTPS using Cloudflare Origin Certificates:',
      cfStep1: 'In Cloudflare dashboard, go to SSL/TLS > Origin Server.',
      cfStep2: 'Click "Create Certificate". Choose RSA, set hostnames and validity (15 years).',
      cfStep3: 'Copy the Origin Certificate and Private Key.',
      cfStep4: 'Save them as .pem files and upload above.',
      cfStep5: 'Set Cloudflare SSL/TLS mode to "Full (Strict)".',
      cfNote: 'Cloudflare Origin Certificates are only trusted by Cloudflare proxy, not browsers directly.',
    },
    adblock: {
      title: 'Ad block',
      subtitle: 'Route ad-network and tracker domains to the block outbound on both engines.',
      howItWorks: 'When enabled, the panel adds a managed routing rule that sends geosite:category-ads-all traffic to the block outbound. The proxy engines reload automatically.',
      youtubeCaveat: "Note: YouTube serves in-video ads from the same CDN as the videos themselves (server-side ad insertion), so this rule can't strip them. For DOM-level YouTube ad-stripping, use uBlock Origin in your browser, SmartTube on Android TV, or ReVanced on mobile.",
      toastEnabled: 'Ad block enabled',
      toastDisabled: 'Ad block disabled',
    },
    acme: {
      title: "Let's Encrypt (ACME)",
      subtitle: 'Issue a real TLS certificate for any domain that resolves to this VPS. Renews automatically every 30 days.',
      domainLabel: 'Domain',
      emailLabel: 'Email',
      issueBtn: 'Issue certificate',
      issuing: 'Issuing…',
      portWarn: 'Port 80/tcp must be reachable from the public Internet during issuance (HTTP-01 challenge). Open it in your firewall before clicking.',
      errorMissing: 'Both domain and email are required',
      issued: 'Certificate issued',
      successHeader: 'Certificate ready',
      expires: 'Expires',
      nextStep: 'Wire these paths into an inbound\'s TLS settings, or upload them above to enable HTTPS on the panel itself.',
    },
    backup: {
      title: 'Backup & Restore',
      subtitle: 'Export or restore your panel state',
      desc: 'Export a portable archive of inbounds, clients, routing rules, cron jobs, and non-secret settings. Restore from the archive to migrate to another VPS or roll back after a misconfiguration. Admin credentials, JWT secret, and TLS paths are never exported — your login is preserved.',
      export: 'Export Backup',
      exporting: 'Exporting…',
      restore: 'Restore from File',
      restoring: 'Restoring…',
      exported: 'Backup downloaded',
      restored: 'Backup restored',
      confirmTitle: 'Restore from backup?',
      confirmRestore: 'This will replace all current inbounds, clients, routing rules, and cron jobs with the archive contents. Continue?',
    },
    apiTokens: {
      title: 'API Tokens',
      subtitle: 'Long-lived credentials for the zenithctl CLI and automation',
      nameLabel: 'Name',
      namePlaceholder: 'ci-runner',
      scopesLabel: 'Scopes',
      expiresLabel: 'Expires in days (0 = never)',
      create: 'Create token',
      creating: 'Creating…',
      errorNameRequired: 'Token name is required',
      empty: 'No tokens yet. The first token you create will show its plaintext once and only once.',
      colName: 'Name',
      colScopes: 'Scopes',
      colExpiry: 'Expires',
      colLastUsed: 'Last used',
      colStatus: 'Status',
      colActions: 'Actions',
      revoke: 'Revoke',
      revoked: 'revoked',
      active: 'active',
      revokeConfirmTitle: 'Revoke this token?',
      revokeConfirmBody: 'Calls authenticated with "{name}" will start failing immediately. The row stays in the audit list.',
      revokedToast: 'Token revoked',
      justCreated: 'Token "{name}" created',
      copyNowWarn: 'Copy this token now — it will never be shown again. Anyone holding it can act on the panel.',
      copy: 'Copy',
      dismiss: 'Dismiss',
      copied: 'Copied to clipboard',
    },
    auth: {
      title: 'Authentication',
      subtitle: 'Manage your passwords and two-factor auth',
      panelPassword: 'Panel Password',
      panelPasswordDesc: 'Change your admin password',
      oldPassword: 'Current Password',
      newPasswordField: 'New Password (min 8 chars)',
      confirmNewPassword: 'Confirm New Password',
      savePassword: 'Save Password',
      passwordChanged: 'Password changed successfully!',
      errorEmpty: 'Please fill in all password fields',
      errorMismatch: 'New passwords do not match',
      errorShort: 'New password must be at least 8 characters',
      twoFactor: 'Two-Factor Authentication (2FA)',
      twoFactorEnabled: 'Enabled',
      twoFactorDisabled: 'Not configured',
      enable2fa: 'Enable 2FA',
      disable2fa: 'Disable 2FA',
      scanQR: 'Scan this QR code with your authenticator app (Google Authenticator, Authy, etc.)',
      manualEntry: 'Or enter this secret manually:',
      enterCode: 'Enter the 6-digit verification code',
      verifyCode: 'Verify & Enable',
      recoveryCodesTitle: 'Save Your Recovery Codes',
      recoveryCodesDesc: 'Store these codes in a safe place. Each code can only be used once to login if you lose access to your authenticator.',
      recoveryCodesSaved: 'I have saved these recovery codes',
      confirmDisable2fa: 'Enter your password to disable 2FA:',
    },
    tips: {
      title: 'Security Tips',
      intro: 'Keep your panel secure:',
      strongPassword: 'Use a strong admin password',
      enable2fa: 'Enable two-factor authentication',
      keepUpdated: 'Keep the panel updated',
      useHttps: 'Use HTTPS with a valid certificate',
      restrictApi: 'Restrict API access by IP if possible',
      portLabel: 'Current Panel Port',
      portWarning: 'You are using a commonly-scanned port (80, 8080, etc.). Consider changing to a random high port (10000+) for better security.',
      portCF: 'Using a Cloudflare-compatible HTTPS port. Good choice if your domain is proxied through Cloudflare.',
      portGood: 'Good — using a non-standard port.',
    },
    cloudflare: {
      title: 'Cloudflare Protection',
      subtitle: 'Restrict panel access to Cloudflare proxy only',
      desc: 'When enabled, only Cloudflare servers can reach your panel port. Direct IP access from anyone else will be blocked by the firewall.',
      howItWorks: 'How it works:',
      step1: 'Adds ACCEPT rules for all Cloudflare IPv4 ranges on port {port}',
      step2: 'Adds a DROP rule blocking all other IPs on that port',
      enable: 'Enable Cloudflare Protection',
      disable: 'Disable Cloudflare Protection',
    },
  },

  // System Optimization
  optimize: {
    title: 'System Optimization',
    subtitle: 'One-click performance tuning for your server.',
    bbr: {
      title: 'BBR Congestion Control',
      subtitle: 'TCP optimization for better network throughput',
      desc: 'BBR (Bottleneck Bandwidth and RTT) is a TCP congestion control algorithm developed by Google. It significantly improves network throughput and reduces latency, especially for proxy traffic.',
      current: 'Current Algorithm',
      available: 'Available Algorithms',
      enable: 'Enable BBR',
      disable: 'Disable BBR',
      kernelNote: 'Requires Linux kernel 4.9 or later.',
    },
    swap: {
      title: 'Swap Memory',
      subtitle: 'Virtual memory for low-RAM servers',
      desc: 'Create a swap file to extend available memory. Essential for VPS with 512MB-1GB RAM to prevent out-of-memory crashes.',
      totalMb: 'Total Swap',
      usedMb: 'Used',
      filePath: 'Swap File',
      sizeLabel: 'Swap Size',
      create: 'Create Swap',
      remove: 'Remove Swap',
      creating: 'Creating...',
      confirmRemove: 'Remove swap file? This may cause issues if memory is low.',
    },
    sysctl: {
      title: 'Network Tuning',
      subtitle: 'Kernel-level network parameter optimization',
      desc: 'Applies a set of optimized kernel parameters: larger TCP buffers, fast open, connection reuse, and higher backlog limits. Ideal for proxy servers.',
      enable: 'Apply Tuning',
      disable: 'Revert to Defaults',
      params: 'Current Parameters',
    },
    cleanup: {
      title: 'System Cleanup',
      subtitle: 'Free disk space by removing caches and logs',
      desc: 'Clean up systemd journal logs (keep 3 days), package manager cache, and unused Docker images/containers.',
      journal: 'Journal Logs',
      package: 'Package Cache',
      docker: 'Docker Reclaimable',
      scan: 'Scan',
      scanning: 'Scanning...',
      run: 'Run Cleanup',
      running: 'Cleaning...',
      freedJournal: 'Journal freed',
      freedPackage: 'Package cache',
      freedDocker: 'Docker freed',
      confirmRun: 'This will permanently delete old logs, caches, and unused Docker data. Continue?',
    },
  },

  // Traffic Monitor (live)
  traffic: {
    title: 'Traffic Monitor',
    subtitle: 'Live view of proxy users and VPS process activity',
    lastUpdated: 'Updated: {time}',
    waiting: 'Waiting for data…',
    fetchFailed: 'Failed to fetch traffic',
    noExportData: 'Nothing to export yet',
    csvFilePrefix: 'zenith-traffic',
    tabs: {
      proxy: 'Proxy Users',
      system: 'System Processes',
    },
    totalUp: 'Total Upload Rate',
    totalDown: 'Total Download Rate',
    activeConns: 'Active Connections',
    engine: 'Engine',
    protocol: 'Protocol',
    showingUsers: 'Showing {shown} / {total} users',
    table: {
      user: 'User',
      engineProtocol: 'Engine / Protocol',
      up: 'Up',
      down: 'Down',
      conns: 'Conns',
      totalUp: 'Total Up',
      totalDown: 'Total Down',
      recentTargets: 'Recent Targets',
    },
    unknown: 'Unknown',
    noMatch: 'No users match the current filters',
    proxyDisabled: 'Proxy or Clash API not enabled — see the note above',
    noActiveUsers: 'No active users',
    system: {
      nicTotalRate: 'Total NIC Rate (all interfaces)',
      inbound: 'In',
      outbound: 'Out',
      total: 'Total',
      activeNetProcs: 'Active Network Processes',
      procHint: 'Sorted by open socket count. Refreshed every 5s (independent of NIC rate). Per-process live bandwidth needs nethogs/eBPF and is not shown here.',
      pid: 'PID',
      process: 'Process',
      user: 'User',
      conns: 'Conns',
      listenPorts: 'Listen Ports',
      targets: 'Targets',
      noNetProcs: 'No active network processes',
    },
  },

  // Egress traffic history
  egress: {
    title: 'Egress Traffic',
    subtitle: 'Egress & return traffic history by proxy instance / user / destination (domain · IP · ASN)',
    updatedAt: 'Updated {time}',
    settings: 'Settings',
    loadFailed: 'Failed to load egress traffic',
    csvFilePrefix: 'zenith-egress',
    exportScopeDetail: 'Export detail rows',
    exportScopeDests: 'Export destinations',
    ranges: { '1h': '1h', '6h': '6h', '24h': '24h', '7d': '7d', '30d': '30d' },
    directions: { egress: 'Egress', return: 'Return', all: 'All' },
    allInstances: 'All instances',
    filterUser: 'Filter by user email…',
    config: {
      title: 'Collection Settings',
      enabled: 'Enable egress traffic collection',
      socketSampler: 'ss socket sampler (all services)',
      asn: 'ASN / country DNS resolution',
      rdns: 'Reverse-DNS domain backfill (IP-only destinations)',
      retentionDays: 'Detail retention (days)',
      hourlyRetentionDays: 'Hourly rollup retention (days)',
      pruneHour: 'Prune hour (0-23)',
      xrayAccessPath: 'zenith-xray access.log path (empty = domain collection off)',
      saved: 'Saved',
      saveFailed: 'Save failed',
    },
    cards: {
      total: 'Range Total',
      upDown: 'Up / Down',
      dests: 'Destinations',
      activeInstances: 'Active Instances',
    },
    series: {
      title: 'Traffic Time Series (stacked by instance)',
      empty: 'No data in this range (collection flushes every 30s; allow a moment after enabling)',
    },
    coverage: {
      title: 'Per-Instance Data Fidelity (honestly labeled)',
      domain: 'Domain',
      user: 'User',
      bytes: 'Bytes',
    },
    topDests: {
      title: 'Top Destinations (domain / IP)',
      empty: 'No data',
      unresolved: '(unresolved)',
      rdnsHint: ' (reverse-DNS guess, for reference only)',
    },
    topAsns: {
      title: 'Top ASN / Org',
      empty: 'No data (ASN resolves asynchronously, appears later)',
    },
    byInstance: { title: 'By Instance', unknown: '(unknown)' },
    byUser: { title: 'By User', empty: 'No per-user data (IP-only instances have no users)' },
    table: {
      title: 'Detail (latest 200 bucket records)',
      time: 'Time',
      instance: 'Instance',
      user: 'User',
      dest: 'Destination',
      asn: 'ASN',
      direction: 'Direction',
      up: 'Up',
      down: 'Down',
      empty: 'No records',
    },
  },

  // Smart Deploy wizard
  smartDeploy: {
    title: 'Smart Deploy',
    subtitle: 'From zero to a working egress tunnel in one flow: probe → preset → tuning → protocol deploy → subscription.',
    steps: { probe: 'Probe', preset: 'Preset', options: 'Options', preview: 'Preview', done: 'Done' },
    probing: 'Probing environment…',
    probeFailed: 'Probe failed: {err}',
    retry: 'Retry',
    chips: {
      kernel: 'Kernel',
      bbr: 'BBR',
      perms: 'Privileges',
      distro: 'Distro',
      publicIp: 'Public IPv4',
      cpuRam: 'CPU / RAM',
      primaryNic: 'Primary NIC',
      timeSync: 'Time Sync',
      firewall: 'Firewall',
      port443: 'Port 443',
      port8443: 'Port 8443',
      systemd: 'Systemd',
    },
    values: {
      unknown: 'Unknown',
      available: 'Available',
      unavailable: 'Unavailable',
      notDetected: 'Not detected',
      cores: 'cores',
      unrecognized: 'Unrecognized',
      free: 'Free',
      occupied: 'In use',
      missing: 'Missing',
      present: 'present',
    },
    blockers: {
      title: 'Blockers',
      notRoot: 'The panel is not running as root; system tuning cannot be applied.',
      noSystemd: 'systemd not detected; Phase 1 does not support SysV init.',
      noPublicIp: 'Could not detect a public IPv4 address.',
    },
    warnings: {
      title: 'Warnings',
      timeNotSynced: 'System time is not synced; TLS handshakes may fail. Enable chronyd or systemd-timesyncd first.',
      noBbr: 'The kernel has no BBR module; tuning gains will be limited.',
      port443Occupied: 'Port 443 is already in use; the panel will auto-pick a fallback (8443, etc.).',
    },
    reprobe: 'Re-probe',
    nextPreset: 'Next: choose preset',
    recommended: 'Recommended',
    prev: 'Back',
    nextOptions: 'Next: optional parameters',
    options: {
      domain: 'Domain (optional)',
      domainHint: '— providing a domain issues a real certificate via ACME; leave empty for self-signed',
      email: 'Email (for ACME registration)',
      realitySni: 'Reality target SNI (optional)',
      realitySniHint: '— defaults to www.microsoft.com',
      portOverride: 'Port override (optional)',
      portOverrideHint: '— leave empty to use 443 or auto-fallback',
    },
    generating: 'Generating preview…',
    generatePreview: 'Generate preview',
    preview: {
      title: 'Operations to perform',
      preset: 'Preset:',
      certMode: 'Certificate mode:',
      inbounds: 'Inbounds ({n}):',
      tuning: 'Tuning operations ({n}):',
      firewallPorts: 'Firewall allow ports:',
      notes: 'Notes',
      modifyOptions: 'Modify options',
    },
    deploying: 'Deploying…',
    confirmDeploy: 'Confirm & deploy',
    result: {
      success: '✓ Deployment succeeded',
      deployId: 'Deployment ID: {id}',
      failPrefix: '× Deployment failed · status: {status}',
      unknownError: 'Unknown error',
      autoRolledBack: 'Applied operations were automatically rolled back on failure.',
    },
    rollback: 'Roll back',
    rollbackConfirm: 'Roll back this deployment? All tuning and inbounds from this run will be reverted.',
    newDeploy: 'New deployment',
  },

  // Language
  language: {
    title: 'Language',
  },
}
