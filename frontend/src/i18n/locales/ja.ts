  ...
  proxy: {
    ...
    quickSetup: {
      ...
      fields: {
        ...
        shortId: 'Short ID',
        fingerprint: 'フィンガープリント',
        domain: 'ドメイン（TLS 証明書用、任意）',
        domainRequired: '任意。空欄 → 自己署名証明書 + サブスクリプション URL に `insecure=1` を自動付与（クライアント側で証明書検証をスキップする必要があります）。このサーバーを指すドメインを入力 → Let\'s Encrypt 証明書で厳格 TLS 検証。',
