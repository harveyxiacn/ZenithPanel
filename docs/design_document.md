...
### 2.1 部署架构
- **单文件二进制运行**: 将包含 Vue/React 构建产物的前端静态资源，通过 Go 1.16+ 的 `go:embed` 技术打包进后端的同一个二进制可执行文件中。用户无需配置复杂的 Nginx+PHP 或 Nginx+Node 环境，开箱即用。
...
- **防止后门**: 所有引入的第三方包须严格经过安全扫描并且锁定版本号 (`go.sum` / `pnpm-lock.yaml`)。
