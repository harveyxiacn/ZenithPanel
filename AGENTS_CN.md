...
通配 DNS：`<带横杠的-IP>.nip.io` 自动解析到对应 IP。VPS `203.0.113.10`：
...
| nip.io / sslip.io | 只有公网 IP | `203-0-113-10.nip.io` | `zenithctl cert issue --domain 203-0-113-10.nip.io`（免费真实 LE 证书） |
...
- **阶段 2（真实 LE 证书）：** 执行 `zenithctl cert issue --domain
  203-0-113-10.nip.io` 之后，把 Hy2 入站改为 `serverName=
  "203-0-113-10.nip.io"`，指向 LE 证书文件，订阅 URL 去掉
...
203-0-113-10.nip.io`。可视化编辑器对 Hy2/TUIC 的 SNI 字段加红色
...
换新镜像后，swap helper 会在删掉旧容器后跑一次 `docker image prune -f`
（只清 dangling，不带 `-a`）——被顶掉 `:main` 标签的旧面板镜像会被回收，
小盘 VPS 反复 OTA 不会被旧镜像塞满；其他容器的带 tag 镜像不受影响。
