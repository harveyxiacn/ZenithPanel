#!/bin/bash
# Drives one sing-box client per inbound, opens a SOCKS5 on a different local
# port for each, curls google through it, and prints a single-line pass/fail.
# Each client config is generated in-place so adding a protocol later is just
# appending a `run_one` line.
set -u
...
# ---------------- TUIC v5 client ----------------
TUIC_UUID=$(zenithctl -q raw GET /api/v1/clients | python3 -c "import json,sys; print(next((c['uuid'] for c in json.load(sys.stdin) if c['inbound_id']==6), ''))")
cat > /root/proto-tests/sb-tuic.json <<JSON
{
  ...
  "outbounds": [{
    ...
    "uuid": "$TUIC_UUID", "password": "$TUIC_UUID",
